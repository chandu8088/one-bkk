;(function (global, factory) {
    ...
    DragScrollable.prototype = {
        ...
        _moveHandler: function _moveHandler(event) {
            ...
            var maxAvailableLeft = (content.currentWidth - window.originalWidth) / 2 + content.correctX
            var maxAvailableTop = (content.currentHeight - window.originalHeight) / 2 + content.correctY // if we do not go beyond the permissible boundaries of the window

            if (Math.abs(contentNewLeft) <= maxAvailableLeft) content.currentLeft = contentNewLeft // if we do not go beyond the permissible boundaries of the window
    ...
    function _transform($element, _ref, options) {
        ...
        if (options.smoothExtinction) {
            $element.style.transition = 'transform '.concat(options.smoothExtinction, 's')
        } else {
            $element.style.removeProperty('transition')
        }

        $element.style.transform = 'translate3d('.concat(left, 'px, ').concat(top, 'px, 0px) scale(').concat(scale, ')')
    }
    ...
    WZoom.prototype = {
        ...
        _prepare: function _prepare() {
            ...
            this.content.minScale =
                ...
                Math.min(this.window.originalWidth / this.content.originalWidth, this.window.originalHeight / this.content.originalHeight)
            ...
            var _calculateAlignPoint2 = _calculateAlignPoint(this.options, this.content, this.window),
                _calculateAlignPoint3 = _slicedToArray(_calculateAlignPoint2, 2),
            ...
            var _calculateCorrectPoin = _calculateCorrectPoint(this.options, this.content, this.window),
                _calculateCorrectPoin2 = _slicedToArray(_calculateCorrectPoin, 2),
        ...
        _computeNewScale: function _computeNewScale(delta) {
            ...
            var contentNewScale = currentScale + this.direction / this.options.speed
        ...
        _computeNewPosition: function _computeNewPosition(contentNewScale, _ref) {
            var x = _ref.x,
                y = _ref.y
            var window = this.window,
                content = this.content
            ...
            if (this.direction === -1) {
                ...
                contentNewLeft = _calculateContentMaxShift(this.options, window.originalWidth, content.correctX, contentNewWidth, contentNewLeft) // check that the content does not go beyond the Y axis
        ...
        _transform: function _transform(_ref2) {
            ...
            if (this.options.smoothExtinction) {
                this.content.$element.style.transition = 'transform '.concat(this.options.smoothExtinction, 's')
            } else {
                this.content.$element.style.removeProperty('transition')
            }

            this.content.$element.style.transform = 'translate3d('.concat(newLeft, 'px, ').concat(newTop, 'px, 0px) scale(').concat(newScale, ')')
    ...
    function _calculateCorrectPoint(options, content, window) {
        var correctX = Math.max(0, (window.originalWidth - content.currentWidth) / 2)
        var correctY = Math.max(0, (window.originalHeight - content.currentHeight) / 2)
